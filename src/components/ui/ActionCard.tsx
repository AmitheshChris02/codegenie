...
type Props = {
  ...
  actions: A2UIAction[];
  onAction: (action: A2UIAction) => Promise<void>;
};

function buttonClass(style?: string) {
  if (style === "primary") return "bg-brand-600 text-white border-brand-600 hover:bg-brand-700";
  if (style === "danger") return "border-red-400 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20";
        ...
        {actions.map((action) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm transition disabled:opacity-70 ${buttonClass(action.style)}`}
            >
